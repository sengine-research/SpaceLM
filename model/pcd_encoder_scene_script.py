...
conv3d_config = F.conv_config.get_default_conv_config()
conv3d_config["kmap_mode"] = "hashmap"
...
def make_conv3d_sparse(
    ...
    block = nn.Sequential(
        spnn.Conv3d(channels_in, channels_out, kernel_size=kernel_size, stride=1, config=conv3d_config),
...
def make_conv3d_downscale_sparse(
    ...
    num_groups = min(num_groups, channels_out)
    block = nn.Sequential(
        spnn.Conv3d(channels_in, channels_out, kernel_size=2, stride=2, config=conv3d_config),
        spnn.GroupNorm(num_groups, channels_out),
        activation(inplace=True),
    )
    return block
...
class PointCloudEncoder(nn.Module):
    def __init__(
        ...
        self.point_proj = nn.Linear(d_model, token_hidden_size)
    ...
    def forward(self, point_cloud: torchsparse.SparseTensor):
        ...
        context = self.point_proj(context)
