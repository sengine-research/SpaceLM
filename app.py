...
from SLAM3R.recon import get_img_tokens, initialize_scene, adapt_keyframe_stride, i2p_inference_batch, l2w_inference, normalize_views, scene_frame_retrieve
from SLAM3R.slam3r.datasets.wild_seq import Seq_Data
from SLAM3R.slam3r.models import Local2WorldModel, Image2PointsModel
from SLAM3R.slam3r.utils.device import to_numpy
from SLAM3R.slam3r.utils.recon_utils import *
...
def recon_scene(i2p_model:Image2PointsModel, 
    ...
    save_path_glb, save_path_ply = get_model_from_scene(per_frame_res=per_frame_res, 
                                     save_dir=save_dir, 
                                     num_points_save=num_points_save, 
                                     conf_thres_res=conf_thres_l2w)

    return save_path_glb, save_path_ply, per_frame_res
...
def get_model_from_scene(per_frame_res, save_dir, 
    ...
    save_name_glb = f"recon.glb"
    save_name_ply = f"recon.ply"
    scene = trimesh.Scene()
    scene.add_geometry(trimesh.PointCloud(vertices=sampled_pts, colors=sampled_rgbs/255.))
    save_path_glb = join(save_dir, save_name_glb)
    save_path_ply = join(save_dir, save_name_ply)
    scene.export(save_path_glb)
    trimesh.PointCloud(vertices=sampled_pts, colors=sampled_rgbs/255.).export(save_path_ply)

    return save_path_glb, save_path_ply
...
def main_demo(i2p_model, l2w_model, device, tmpdirname, server_name, server_port):
    ...
    with gradio.Blocks(css=""".gradio-container {margin: 0 !important; min-width: 100%};""", title="SLAM3R Demo") as demo:
        ...
        with gradio.Column():
            ...
            outmodel_glb = gradio.Model3D()
            outmodel_ply = gradio.File(file_count="single", file_types=["ply"],
                                      scale=2,
                                      height=200,
                                      label="output pointcloud in ply format")
